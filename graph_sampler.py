...
class GraphSampler(torch.utils.data.Dataset):
    ...
    def __init__(self, G_list, features='default', normalize=True, assign_feat='default', max_num_nodes=0):
        ...
        for G in G_list:
            ...
            if normalize:
                ...
                nor_adj = np.matmul(np.matmul(sqrt_deg, adj), sqrt_deg)
                self.nor_adj_all.append(nor_adj)
            else:
                self.nor_adj_all.append(np.zeros((adj.shape), dtype=float))
    ...
    def __getitem__(self, idx):
        ...
        nor_adj = self.nor_adj_all[idx]
        num_nodes = adj.shape[0]
        num_edges = np.sum(adj)/2
        adj_padded = np.zeros((self.max_num_nodes, self.max_num_nodes))
        adj_padded[:num_nodes, :num_nodes] = adj
        nor_adj_padded = np.zeros((self.max_num_nodes, self.max_num_nodes))
        nor_adj_padded[:num_nodes, :num_nodes] = nor_adj

        # use all nodes for aggregation (baseline)

        return {'adj':adj_padded,
                'nor_adj':nor_adj_padded,
