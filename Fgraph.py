...
class GraphConv(nn.Module):
    ...
    #     D = [torch.sum(adj[i], 1) for i in range(adj.size(0))]
    #     D1 = [torch.diag(torch.pow(D[i], D[i].new_full((adj.size(1),),-0.5))).unsqueeze(0).cuda() for i in range(adj.size(0))]
    #     D1 = torch.cat([D1[i]] for i in range(adj.size(0)))
...
class SoftPoolingGcnEncoder(GcnEncoderGraph):
    def __init__(self, max_num_nodes, input_dim, hidden_dim, embedding_dim, label_dim, num_layers,
        ...
        self.read_first_after_pool, self.read_block_after_pool, self.read_last_after_pool = self.build_conv_layers(
            assign_dim, self.hidden, 1, num_layers,
            add_self, normalize=False, dropout=dropout)
        # GC
        ...
        #     label_dim, num_aggs=self.num_aggs)
        self.pred_model = self.build_pred_layers(
            self.pred_input_dim * (num_pooling + 2) if pool_concat else self.pred_input_dim, pred_hidden_dims,
            label_dim, num_aggs=self.num_aggs)
    ...
    def forward(self, x, adj, batch_num_nodes, **kwargs):
        ...
        self.x1 = embedding_tensor
        # self.a1 = [self.x1[i].cpu().detach().numpy() for i in range(10)]

        self.x1_ = torch.cat([torch.t(self.x1[i]).unsqueeze(0) for i in range(self.batch_size)])

        self.fea_A1 = self.get_fea_A(self.x1, self.args.batch_size, self.ratio)
        ...
        for i in range(self.num_pooling):
            if batch_num_nodes is not None and i == 0:
                embedding_mask = self.construct_mask(self.max_num_nodes, batch_num_nodes)
            else:
                embedding_mask = None
            # print(x_a.shape)
            ...
            x_a = x
            self.x_a = x_a
            self.x_a_ = [self.x_a[i].cpu().detach().numpy() for i in range(self.batch_size)]
            ...
            self.fea_A2 = self.get_fea_A(self.x2, self.args.batch_size, self.ratio)
        ...
        if self.pool_concat:
            if self.read_gnn:
                ...
                # output = self.out1 + self.out2
                self.output = [output[i].cpu().detach().numpy() for i in range(self.batch_size)]
            else:
                output = torch.cat(out_all, dim=1)
                self.output = [output[i].cpu().detach().numpy() for i in range(self.batch_size)]
        else:
