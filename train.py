...
if __name__ == "__main__":
    ...
    # # with open(args.logdir, gen_prefix(args), 'val_adj_grads.pkl', 'rb') as f:
    # #     adj_grads = pkl.load(f)
    # logname = 'log/PROTEINS_soft-assign_l3x1_ar10_h64_o64_201_dp10_pcT_lkF/'
    # logname1 = 'log/PROTEINS_soft-assign_l3x1_ar10_h64_o64_201_dp10_pcF_lkF/'
    # with open(logname + 'val_adj_grads.pkl', 'rb') as f:
    #     adj_grads_M = pkl.load(f)
    # with open(logname1 + 'val_adj_grads.pkl', 'rb') as f:
    #     adj_grads = pkl.load(f)
    #
    # grads_M = adj_grads_M['grad']
    # grads = adj_grads['grad']
    # large_num = 0
    # a = []
    # for i in range(len(grads)):
    #     a.append([np.max(np.reshape(grads_M[i], -1)) / max(np.reshape(grads[i], -1)),
    #               min(np.reshape(grads_M[i], -1)) / min(np.reshape(grads[i], -1))])
    #     # if np.max(np.reshape(grads_M[i],-1))<max(np.reshape(grads[i],-1)) and min(np.reshape(grads_M[i],-1))>min(np.reshape(grads[i],-1)):
    #     #     large_num +=1
    #     #     a.append([np.max(np.reshape(grads_M[i],-1))/max(np.reshape(grads[i],-1)),min(np.reshape(grads_M[i],-1))/min(np.reshape(grads[i],-1))])
    ...
    for i in range(1):  # 10
        ...
        for ri in range(len(attack_ratio)):
            ...
            with open('results/test_adj_update_{}.pkl'.format(8), 'rb') as f:
                test_update_adjs_all = pickle.load(f)
            with open('results/test_adj_update_{}.pkl'.format(0), 'rb') as f:
                ori_adjs_all = pickle.load(f)
            test_aug_adjs_all = []

            with open('/home/linxiang/lyw/tfl/diffpool-proteins-attack/log/PROTEINS_soft-assign_l3x1_ar10_h64_o64_201_dp10_pcT_lkF/val_adj_grads.pkl', 'rb') as gf:
                grad_ = pickle.load(gf)
                grad = grad_['grad']

            # batch_num_nodes = data['num_nodes'].int().numpy()
            # test_aug_adjs_batch = test_update_adjs_all[1].clone()
            # adj = np.array(test_aug_adjs_batch[0].cpu())
            # ct_c, num_edges = cent_calc(adj)
            # c_dict = {}
            # ct_c = ct_c[0:batch_num_nodes[i], 0:batch_num_nodes[i]]
            graph = 1
            ...
            for batch_idx, data in enumerate(val_dataset):
                batch_num_nodes = data['num_nodes'].int().numpy()
                test_aug_adjs_batch = test_update_adjs_all[batch_idx].clone()
                # adj = np.array(test_aug_adjs_batch[graph].cpu())
                adj_g = test_aug_adjs_batch
                ...
                ypred = model(h0, adj_g, batch_num_nodes, assign_x=assign_input)
                if not args.method == 'soft-assign' or not args.linkpred:
                    loss = model.loss(ypred, label)
                else:
                    loss = model.loss(ypred, label, adj_g, batch_num_nodes)
                loss.backward(retain_graph=True)
                grads = model.update_adj().grad
                # g = grads[graph][0:batch_num_nodes[graph], 0:batch_num_nodes[graph]]
                # grad_t = g + torch.transpose(g, 0, 1)
                # grad_g = (np.array(grad_t.cpu()) / 2.)
                grad_g = []
                for i in range(test_aug_adjs_batch.size()[0]):
                #     g = grads[i][0:batch_num_nodes[i], 0:batch_num_nodes[i]]
                #     grad_t = g + torch.transpose(g, 0, 1)
                #     grad_g = np.array(grad_t.cpu()) / 2.
                    g = grads[i][0:batch_num_nodes[i], 0:batch_num_nodes[i]]
                    ...
                    for r in range(batch_num_nodes[i]):
                        for c in range(batch_num_nodes[i]):
                            if r < c and ct_c[r][c] != 0:
                                c_dict[(r, c)] = ct_c[r, c].item()
                    c_sorted = sorted(c_dict.items(), key=lambda x: x[1], reverse=True)

                    ori_adj = np.array(ori_adjs_all[batch_idx][i].cpu())
                    ori_ct_c, num_edges = cent_calc(ori_adj)
                    ori_c_dict = {}
                    ori_ct_c = ori_ct_c[0:batch_num_nodes[i], 0:batch_num_nodes[i]]
                    for r in range(batch_num_nodes[i]):
                        for c in range(batch_num_nodes[i]):
                            if r < c and ori_ct_c[r][c] != 0:
                                ori_c_dict[(r, c)] = ori_ct_c[r, c].item()
                    ori_c_sorted = sorted(ori_c_dict.items(), key=lambda x: x[1], reverse=True)

                    g_ori = abs(grad[10*batch_idx+i] * (ori_adj[0:batch_num_nodes[i], 0:batch_num_nodes[i]]))
                    g_att = abs(grad_g * (adj[0:batch_num_nodes[i], 0:batch_num_nodes[i]]))

                    c_coef_ori = (np.corrcoef(g_ori.flatten(), ori_ct_c.flatten())[0][1])
                    c_coef_att = (np.corrcoef(g_att.flatten(), ct_c.flatten())[0][1])
                    c_ori.append(c_coef_ori)
                    c_att.append(c_coef_att)
                ...
                # ct_c, num_edges = cent_calc(adj)
                # c_dict = {}
                # ct_c = ct_c[0:batch_num_nodes[graph], 0:batch_num_nodes[graph]]
                # for r in range(batch_num_nodes[graph]):
                #     for c in range(batch_num_nodes[graph]):
                #         if r < c and ct_c[r][c] != 0:
                #             c_dict[(r, c)] = ct_c[r, c].item()
                # c_sorted = sorted(c_dict.items(), key=lambda x: x[1], reverse=True)
                #
                # ori_adj = np.array(ori_adjs_all[batch_idx][graph].cpu())
                # ori_ct_c, num_edges = cent_calc(ori_adj)
                # ori_c_dict = {}
                # ori_ct_c = ori_ct_c[0:batch_num_nodes[graph], 0:batch_num_nodes[graph]]
                # for r in range(batch_num_nodes[graph]):
                #     for c in range(batch_num_nodes[graph]):
                #         if r < c and ori_ct_c[r][c] != 0:
                #             ori_c_dict[(r, c)] = ori_ct_c[r, c].item()
                # ori_c_sorted = sorted(ori_c_dict.items(), key=lambda x: x[1], reverse=True)
                #
                # g_ori = abs( grad[graph] * (ori_adj[0:batch_num_nodes[graph], 0:batch_num_nodes[graph]]))
                # g_att = abs(grad_g * (adj[0:batch_num_nodes[graph], 0:batch_num_nodes[graph]]))
                #
                # c_coef_ori = (np.corrcoef(g_ori.flatten(), ori_ct_c.flatten())[0][1])
                # c_coef_att = (np.corrcoef(g_att.flatten(), ct_c.flatten())[0][1])
                #
                ...
                #
                # a = np.max(ori_ct_c)
                # grad = grad_['grad'][graph]
                # ax = toline(adj)
                # c_ = np.zeros((1,len(ax[1])))
                # g_ = np.zeros((1, len(ax[1])))
                # num = 0
                # for i in range(ori_ct_c.shape[0]):
                #     for j in range(i+1,ori_ct_c.shape[0]):
                #         if ori_ct_c[i][j]!=0:
                #             # c_[0][num] = ori_ct_c[i][j]/( np.max(ori_ct_c) - np.min(ori_ct_c))
                #             # g_[0][num] = (grad[i][j]-np.min(grad))/(np.max(grad) - np.min(grad) )
                #             c_[0][num] = ori_ct_c[i][j]
                #             g_[0][num] = grad[i][j]
                #             num +=1
                # c_coef = (np.corrcoef(c_, g_)[0][1])
                #
                # add_num = 0
                # del_num = 0
                # add_dict = {}
                ...
                #     for c in range(batch_num_nodes[graph]):
                #         if r < c:
                #             if ori_ct_c[r][c] == 0 and ct_c[r][c] != 0:
                #                 add_num +=1
                #                 add_dict[(r, c)] = ct_c[r, c].item()
                #             if ori_ct_c[r][c] != 0 and ct_c[r][c] == 0:
                #                 del_num +=1
                #                 adel_dict[(r, c)] = ori_ct_c[r, c].item()
                # add_c_sorted = sorted(add_dict.items(), key=lambda x: x[1], reverse=True)
                # adel_c_sorted = sorted(adel_dict.items(), key=lambda x: x[1], reverse=True)
                # link_rank = []
                # for i in range(len(add_c_sorted)):
                #     link = add_c_sorted[i]
                #     link_rank.append(c_sorted.index(link)/len(c_sorted))
                # # a = add_c_sorted[0]
                # # a_index = c_sorted.index(a)
                ...
                #
                # preds_att = []
                # # adj = test_aug_adjs_batch
                # h0_att = Variable(data['feats'].float()).cuda()
                # ypred_att = model(h0_att, test_aug_adjs_batch, batch_num_nodes, assign_x=assign_input)
                # _, indices_att = torch.max(ypred_att, 1)
                # preds_att.append(indices_att.cpu().data.numpy())
                # l_att = preds_att[0]
